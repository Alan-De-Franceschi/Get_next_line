...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/12/12 18:39:46 by ade-fran          #+#    #+#             */
/*   Updated: 2023/12/12 18:39:50 by ade-fran         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static char		buf[512][BUFFER_SIZE + 1];
	char			*temp;
	char			*next_line;
	ssize_t			r;
	...
	r = 1;
	temp = NULL;
	if (fd == -1 || BUFFER_SIZE == 0 || read(fd, NULL, 0) < 0 || fd > 511)
		return (NULL);
	if (ft_check_new_line(buf[fd], '\n'))
	{
		temp = ft_strjoin(temp, buf[fd]);
		if (!temp)
			return (NULL);
	}
	temp = ft_readl(fd, temp, &buf[fd], &r);
	if (!temp)
		return (NULL);
	if (r == 0)
		return (temp);
	next_line = ft_get_line(temp, &buf[fd], &next_line);
